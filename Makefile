...
dashboard:                    ## Streamlit dashboard on :8501 (needs the API running)
	SENTINELGUARD_API_BASE_URL=http://localhost:8000 $(RUN)/streamlit run src/sentinelguard/dashboard/app.py --server.port 8501

seed:                         ## scan all bundled samples through the running API
	$(RUN)/python scripts/seed_demo.py http://localhost:8000
